...
def generate_model_config(output_activation='softmax'):
    ...
    if model_config['dropout']:
        # choose dropout rate
        model_config['dropout_rate'] = round(np.random.uniform(MIN_DROPOUT, MAX_DROPOUT), 2)
    ...
    for layer in range(model_config['num_of_layers']):
        model_config['layers'].append({
            ...
            'size': 2 ** np.random.random_integers(MIN_SIZE, MAX_SIZE)
        })
...
for i in range(3):
    ...
    file_name = 'C:\Shmekel\local_repository\Shmekel\Shmekel_Results\default_project\config_{}.json'.format(model_name)
